use jkms::utils::error::{JkmsError, PromptError, StorageError, ExternalError, ValidationError};
use std::io;

#[test]
fn prompt_error_should_display_correctly() {
    let error = JkmsError::Prompt(PromptError::NotFound("test-prompt".to_string()));
    assert_eq!(error.to_string(), "Prompt not found: test-prompt");
    
    let error = JkmsError::Prompt(PromptError::AlreadyExists("existing".to_string()));
    assert_eq!(error.to_string(), "Prompt already exists: existing");
    
    let error = JkmsError::Prompt(PromptError::InvalidFormat("bad format".to_string()));
...
fn storage_error_should_wrap_io_errors() {
    ...
    let error = JkmsError::Storage(StorageError::Io(io_error));
    assert!(error.to_string().contains("IO error"));
    
    let error = JkmsError::Storage(StorageError::ParseError("invalid yaml".to_string()));
    assert_eq!(error.to_string(), "Parse error: invalid yaml");
    
    let error = JkmsError::Storage(StorageError::InvalidPath("bad/path".to_string()));
    assert_eq!(error.to_string(), "Invalid path: bad/path");
}

#[test]
fn external_error_should_handle_clipboard_and_editor() {
    let error = JkmsError::External(ExternalError::ClipboardError("failed to copy".to_string()));
    assert_eq!(error.to_string(), "Clipboard error: failed to copy");
    
    let error = JkmsError::External(ExternalError::EditorError("vim crashed".to_string()));
    assert_eq!(error.to_string(), "Editor error: vim crashed");
}

#[test]
fn validation_error_should_provide_helpful_messages() {
    let error = JkmsError::Validation(ValidationError::InvalidInput("name", "cannot contain spaces".to_string()));
    assert_eq!(error.to_string(), "Invalid input for 'name': cannot contain spaces");
    
    let error = JkmsError::Validation(ValidationError::MissingRequired("template".to_string()));
...
fn error_should_implement_std_error() {
    ...
    let error = JkmsError::Prompt(PromptError::NotFound("test".to_string()));
    assert_error(&error);
}
...
fn error_should_support_conversion_from_io_error() {
    ...
    let jkms_error: JkmsError = io_error.into();
    
    match jkms_error {
        JkmsError::Storage(StorageError::Io(_)) => {} // Expected
...
fn error_should_provide_user_friendly_messages() {
    let error = JkmsError::Prompt(PromptError::NotFound("my-prompt".to_string()));
...
fn error_should_indicate_if_recoverable() {
    let not_found = JkmsError::Prompt(PromptError::NotFound("test".to_string()));
    ...
    let storage_error = JkmsError::Storage(StorageError::Io(io_error));
